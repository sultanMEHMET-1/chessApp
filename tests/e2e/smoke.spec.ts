...
test('smoke: move shows in history and analysis refreshes', async ({ page }) => {
  ...
  await expect(page.getByTestId(ANALYSIS_LINE_1)).toBeVisible();
  await expect(page.getByTestId(ANALYSIS_LINE_2)).toBeVisible();
  await expect(page.getByTestId(ANALYSIS_LINE_3)).toBeVisible();
